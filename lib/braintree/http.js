...
var Buffer, Http, Util, braintree, exceptions, forever, foreverOpts, http, https, xml2js;
...
Http = (function() {
  ...
  Http.prototype.httpsAgent = new forever.SSL(foreverOpts);

  Http.prototype.httpAgent = new forever(foreverOpts);
  ...
  Http.prototype.request = function(method, url, body, callback) {
    ...
    options = {
      ...
      headers: {
        'Authorization': 'Basic ' + (new Buffer(this.config.publicKey + ':' + this.config.privateKey)).toString('base64'),
    ...
    options.agent = this.config.environment.ssl ? this.httpsAgent : this.httpAgent;
    if (body) {
